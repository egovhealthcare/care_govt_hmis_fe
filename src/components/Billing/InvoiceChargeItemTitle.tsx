...
/** Minimal interface covering only the fields we access from the batch response. */
interface MedicationDispenseRead {
  item?: {
    product?: {
      product_knowledge?: { name?: string };
...
export function InvoiceChargeItemTitle({
  ...
  const categoryTitle = item.charge_item_definition?.category?.title;
  const resourceLabel = item.service_resource
    ? t(item.service_resource)
    : undefined;
  const subtitle = [resourceLabel, categoryTitle].filter(Boolean).join(" · ");

  // If not a medication dispense, show original title + resource/category
